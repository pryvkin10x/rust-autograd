...
fn get_between_nodes<'t, 'g, T: Float>(
    ...
    while let Some((node, should_visit)) = dfs_stack.pop() {
        if should_visit {
            let has_gradient = node.is_differentiable
                && (is_wrt(node, wrt) || has_marked_child(g.tensor(node.id), &ret));
            ret.insert(node.id(), GradInfo::new(has_gradient, None));
        } else {
            ...
            for child in node.get_backprop_inputs() {
                unsafe {
                    let child = child.get_internal(g);
...
pub(crate) fn symbolic_gradients<'t, 'g, T: Float>(
    ...
    for (y, gy) in ys.iter().zip(gys) {
        between_nodes.get_mut(&y.id()).unwrap().default_grad = Some(gy.id);
    }

    // Prepare a heap with given ys.
    let mut heap = ys.iter().map(|y| y.to_node()).collect::<BinaryHeap<Node>>();
    ...
    while let Some(y) = heap.pop() {
        let gxs = {
            ...
            // clusure は, closure より早く死ぬ可能性があるメソッドないで生成された g をボローする
        ...
        unsafe {
            let xs = g.access_inner(y.id).get_backprop_inputs();
            for (gx, x) in gxs.into_iter().zip(xs) {
                let x = x.get_internal(g);
